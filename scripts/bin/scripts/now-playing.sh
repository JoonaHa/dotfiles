#!/bin/sh
# https://github.com/NapoleonWils0n/ubuntu-bin/blob/master/now-playing
# now-playing
...
if [ -z "${duration}" ]; then
    ...
    notify-send -h string:synchronous:now-playing -i "${icon}" "${title}" "${body}" -u "low"
  else
    body="Artist: ${artist:=${artist_fallback}}\nPlayer: ${player}\nStatus: ${status}\n"
    ...
    notify-send -h string:synchronous:now-playing -h "int:value:${position_percent}" -i "${icon}" "${title}" "${body}" -u "low"
fi
